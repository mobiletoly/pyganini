...
| HTMX forms and request data | Visible HTMX markup with route responses and parsed forms | Visible Jinja `hx-*`, async Starlette forms, and bounded immutable sync request data | `adapt` | [HTMX and forms](htmx-async-forms.md) | Applications own validation, uploads, and response policy | Application code unless another shared wire need appears |
