"""HTTP contract for the bounded React island."""
...
def test_save_contract_and_persistence(client: ExampleClient) -> None:
    ...
    response = _post(
        client,
        b'{"name":"  Calm React editor  ","pinned":true}',
        token=token,
        cookie=cookie,
    ...
    assert response.json() == {"project": {"name": "Calm React editor", "pinned": True}}
    page = client.get("/")
    assert 'data-project-name="Calm React editor"' in page.text
